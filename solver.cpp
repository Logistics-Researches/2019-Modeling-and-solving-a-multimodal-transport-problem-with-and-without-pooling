...
int main(int, char**)
{
	...
	try {
		...
		IloInt ts = rand() % 5 + 3;
		ts = ts * 24 * 60;
		ifstream instance("d:\instance.txt");
		//Lecture des données--------------------------------------------------------------
		//1 n,m,d
		instance >> n;
		instance >> m;
		...
		*/
		...
		/*for (int k = 0; k < m; k++)
		{
			for (i = 0; i < d; i++)
			{
				...
				cont9 += tcd;
				IloConstraint const9(cont9 <= ts);
				const9.setName("Cont9");
				model.add(const9);
		...
		if (cplex.solve())
		{
			...
			for (int i = 0; i < n; i++)
			{
				...
				cplex.getValues(resXnm[i], xnm[i], 1);
			}
			for (int i = 0; i < n; i++)
			{
			...
			for (int i = 0; i < n; i++)
			{
				...
				for (int j = 0; j < m; j++)
				{
					...
					cplex.getValues(resZnm[i][j], znm[i][j], 1);
			...
			for (int i = 0; i < m; i++)
			{
				...
				cplex.getValues(resQmd[i], qmd[i], 1);
			}
			for (int i = 0; i < m; i++)
			{
			...
			for (int i = 0; i < n; i++)
			{
				...
				cplex.getValues(resPnm[i], pnm[i], 1);
			}
			for (int i = 0; i < n; i++)
			{
				for (int mp = 0; mp<m; mp++)
